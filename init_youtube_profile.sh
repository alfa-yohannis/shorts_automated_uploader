#!/usr/bin/env bash
# Initialize the YouTube profile (one-time): COPY your already-signed-in YouTube
# (Google) session from a real Google Chrome profile into profiles/youtube/, then
# open a Chrome window to VERIFY it and flush it to disk. See README §13/§15.
#
# Why copy instead of logging in here: Google captcha-walls fresh logins inside an
# automated/CDP Chrome (same problem as Instagram). The only reliable session is
# one created in your NORMAL Chrome — so sign into youtube.com there first, then
# run this.
#
...
#
#   ./init_youtube_profile.sh                 # copy from Chrome "Default" + verify
#   ./init_youtube_profile.sh "Profile 3"     # copy from a named profile
#   ./init_youtube_profile.sh --no-launch     # copy only, skip the verify window
#
set -euo pipefail
cd "$(dirname "$0")"

CHROME_DIR="$HOME/.config/google-chrome"
DEST="profiles/youtube"
...
# with the DevTools port, on youtube.com — to verify the copied session.
launch_window() {
...
# Verify (read-only over CDP) that the COPIED YouTube session is signed in. The
# login comes from the seeded profile — Google captcha-walls fresh logins inside
...
try:
    ...
    while time.time() < deadline:
        for pg in list(ctx.pages):
            try:
                if "youtube.com" not in pg.url:
                    continue
                # the account avatar button only renders when signed in
                if pg.locator('#avatar-btn, button#avatar-btn, '
                              'ytd-topbar-menu-button-renderer#avatar-btn').count():
                    print("    ✓ YouTube session is signed in.")
                    sys.exit(0)
            except Exception:
                pass
        time.sleep(4)
    print("    (copied profile is NOT signed in — see the instructions above.)")
    sys.exit(1)
finally:
...
# that's what persists the login into profiles/youtube. kill -9 would skip the
# flush and risk losing the just-copied session.
close_and_save() {
    local pid i
...
if [ "$DO_SEED" = 1 ]; then
    ...
    if pgrep -af "/opt/google/chrome/chrome" | grep -vqE "crashpad|--type="; then
        ...
        echo "    1) In Chrome (profile '$SRC_PROFILE'), open youtube.com and confirm" >&2
        echo "       you're signed in (your avatar shows top-right)." >&2
...
PY
    ...
    echo "==> Done. profiles/youtube/ seeded ($(du -sh "$DEST" | cut -f1))."
fi

if [ "$LAUNCH" = 0 ]; then
    echo "==> --no-launch: not opening a window. Run me again without it to verify."
    exit 0
fi
...
==> A Chrome window is open on the YouTube profile to VERIFY the copied session.
    • If youtube.com shows your AVATAR (top-right) — the session copied fine; done.
    • If it shows "Sign in" — your source Chrome profile ('$SRC_PROFILE') wasn't
      signed into YouTube. Do NOT sign in here: Google captcha-walls fresh logins in
      this automated window. Instead:
          1) open your NORMAL Chrome and sign into youtube.com there,
          2) fully close Chrome,
          3) re-run:  ./init_youtube_profile.sh

    Do NOT run ./batch_upload.sh while this window is open — its cleanup kills Chrome.
EOF

echo
echo "==> Verifying the copied YouTube session…"
if wait_for_login; then
    close_and_save
    echo
    echo "==> profiles/youtube is ready (signed in, saved). Upload with:"
    echo "      DISPLAY=$DISPLAY ./venv/bin/python upload_youtube.py <video.mp4> [caption.txt] --chrome --visibility unlisted"
    echo "    (drop --visibility / add --post once you've watched a run go through.)"
else
    echo "==> The copied profile is NOT signed in. Sign into YouTube in your NORMAL"
